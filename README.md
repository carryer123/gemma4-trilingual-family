...
The pipeline is **language-agnostic**: it works for any household with
two distinct parental L1s and a bridge language. KO + RU + EN is the
reference triple included in this release; KO + VI + EN and KO + ZH + EN
extension triples are also generated by `02b_build_multilingual_triples.py`,
and the data scripts can be pointed at any Tatoeba language pair to
support new corridors (e.g. EN+ES, EN+AR, DE+TR, JA+EN, EN+ZH, EN+TL,
EN+HI, ...).
...
# 2. Multilingual dataset (default: KO+RU+EN; add more pairs by editing 01_*)
